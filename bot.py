...
def init_db():
    ...
    # Таблица слотов - для админки (15-минутные)
...
# ===== ФУНКЦИИ ВРЕМЕНИ (С БУФЕРОМ 5 МИНУТ) =====
...
def get_client_time_slots():
    """10-минутные слоты для клиентов (чтобы был буфер)"""
    slots = []
    for hour in range(10, 19):
...
def get_busy_intervals(date_str):
    """Возвращает занятые интервалы с учетом 5-минутного буфера после записи"""
    conn = get_db()
    c = conn.cursor()
    ...
    for row in c.fetchall():
        ...
        # Добавляем 5 минут буфера к концу записи
        busy.append((start_min, end_min + 5))
    conn.close()
    return busy

def get_available_client_times(date_str, duration):
    """Возвращает доступное время для клиента (с учетом буфера)"""
    busy_intervals = get_busy_intervals(date_str)
    all_times = get_client_time_slots()
    available = []
    ...
    for time_str in all_times:
        ...
        # Проверка на конец рабочего дня
        if end_min > 19*60:
            continue
...
def get_dates():
    ...
    for date_str in all_dates:
        ...
        if date_obj >= today:
            ...
            available_times = get_available_client_times(date_str, 5)  # минимальная длительность
            if available_times:
                dates.append({
                    'date': date_str,
...
@app.route('/api/times/<date>')
def get_available_times(date):
    """Возвращает доступное время для клиента (упрощенный список)"""
    # Берем минимальную длительность для первичного отображения
    available = get_available_client_times(date, 5)
