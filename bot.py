...
def create_booking(data):
    ...
    c.execute('''INSERT INTO bookings 
                 (user_id, username, user_name, user_phone, services, total_price, date, time)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
              (data['user_id'], data['username'], data['name'], data['phone'],
...
def get_user_bookings(user_id):
    ...
    c.execute('''SELECT id, date, time, services, total_price 
                 FROM bookings WHERE user_id = ? AND date >= ? ORDER BY date, time''', 
              (user_id, today))
    bookings = c.fetchall()
    conn.close()
    return bookings
...
def cancel_booking(booking_id):
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT date, time, user_id, username, user_name FROM bookings WHERE id = ?", (booking_id,))
    result = c.fetchone()
    if result:
        date, time, user_id, username, user_name = result
        c.execute("DELETE FROM bookings WHERE id = ?", (booking_id,))
        conn.commit()
        conn.close()
        return date, time, user_id, username, user_name
    conn.close()
...
def get_today_bookings():
    ...
    c = conn.cursor()
    today = datetime.datetime.now().strftime("%Y-%m-%d")
    c.execute('''SELECT time, user_name, user_phone, username, services, total_price 
...
def get_all_future_bookings():
    ...
    c = conn.cursor()
    today = datetime.datetime.now().strftime("%Y-%m-%d")
    c.execute('''SELECT date, time, user_name, user_phone, username, services, total_price 
...
@app.route('/api/mybookings', methods=['POST'])
def get_my_bookings():
    data = request.json
...
def book():
    ...
    # Уведомление админу с username
    username = data.get('username', 'нет')
    user_link = f"@{username}" if username and username != 'нет' else f"ID: {data['user_id']}"
...
def admin_commands(message):
    ...
    if text == '📅 Управление слотами':
        ...
        if bookings:
            ...
            for b in bookings:
                user_link = f"@{b[3]}" if b[3] else "нет username"
                msg += f"⏰ {b[0]} - {b[1]}\n📞 {b[2]}\n👤 {user_link}\n💅 {b[4]}\n💰 {b[5]}₽\n➖➖➖\n"
        else:
...
# ----- ОБРАБОТКА АДМИНСКИХ INLINE КНОПОК -----
...
def admin_callback_handler(call):
    ...
    try:
        if call.data.startswith("manage_"):
            ...
            admin_callback_handler(new_call)
